<x-app-layout>
    @include('modal.drinkModal')
    <div class="py-12 min-h-screen bg-white relative">
        <div id="toast-order" class="absolute top-5 right-5 hidden">
            <div class="flex items-center w-full max-w-xs px-4 py-1.5 space-x-4 rtl:space-x-reverse text-gray-500 bg-white divide-x rtl:divide-x-reverse divide-gray-200 rounded-lg shadow dark:text-gray-400 dark:divide-gray-700 space-x dark:bg-gray-800 border border-gray-100" role="alert">
                <img src="{{ asset('images/icons/milktea.png') }}" alt="" class="w-8 h-8">
        ...
        <div class="max-w-6xl mx-auto">            
            <div class="px-1 md:px-0">
                ...
                <div class="mb-2 flex justify-center flex-wrap sm:justify-start md:justify-start">
                    <h1 class="block w-full text-center sm:text-start md:text-start text-2xl font-extrabold text-gray-800 mb-2 ">Milktea</h1>
                    @if(Auth::user()->hasRole('admin'))
                    <button data-modal-target="modal-add-dish" data-modal-toggle="modal-add-dish" class="block text-white bg-yellow-900 hover:bg-yellow-950 font-semibold rounded text-sm px-5 py-1.5 -mt-2" type="button">
                        Add Milktea
                    </button>
                    @endif
                </div>                
                        ...
                        <div class="bg-gray-100 h-60 relative overflow-x-visible overflow-y-clip">
                            <div class="absolute bottom-0 left-0 z-10 w-full button-container bg-white">
                                <div class="grid grid-cols-3 w-full">
                                    <button type="button" class="flex justify-center tooltip text-rose-600 hover:text-rose-600 py-2 text-center hover:bg-gray-100">
                                    ...
                                    <button type="button" class="flex justify-center tooltip text-sky-600 hover:text-sky-600 py-2 hover:bg-gray-100">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <script src="{{ asset('js/user/milktea.js') }}" defer></script>
    <script src="{{ asset('js/user/order.js') }}" defer></script>
    <script src="{{ asset('js/user/cart.js') }}" defer></script>
</x-app-layout>

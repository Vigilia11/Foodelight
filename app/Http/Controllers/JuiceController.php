...
class JuiceController extends Controller
{
    public function index()
    {
        ...
        {
            return view('user.juice');
            
        }
        if(Auth::user()->hasRole('admin'))
        {

            return view('admin.juice');            
        }
    }

    public function fetchJuice(){
        if(Auth::user()->hasRole('user') || Auth::user()->hasRole('admin'))
        {
            $currentDate = Carbon::now()->toDateString();
            
            $juice = DB::table('dishes')                    
    ...
    public function filterJuice($filter){
    ...
    public function fetchJuiceById($id){
    ...
    public function fetchAvailableJuices()
    {
        if(Auth::user()->hasRole('user') || Auth::user()->hasRole('admin'))
        {
            $currentDate = Carbon::now()->toDateString();
        ...
        if(Auth::user()->hasRole('admin')){
            ...
            if($validator->fails())
            {
                ...
                try{
                    ...
                    return response()->json([
                        'status'=>200,
                        'message'=>"Your Juice have been added."
                    ]);
    ...
    public function update(Request $request, $dish_id)
    {
        if(Auth::user()->hasRole('admin')){
            ...
            if($validator->fails())
            {
                ...
                return response()->json([
                    'status'=>200,
                    'message'=>"Your Juice have been updated."
                ]);
            }
        }
    }
